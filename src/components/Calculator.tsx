...
import { DEFAULT_ROLES, PROJECT_SCOPES, BASE_INFRASTRUCTURE_COSTS, USER_COST_MULTIPLIER } from '@/data';
...
const Calculator: React.FC = () => {
  ...
  const handleRateChange = (id: string, rate: number) => {
    setRoles(prev => prev.map(role => 
      role.id === id ? { ...role, hourlyRate: rate } : role
    ));
  };
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Set hourly rates for each role</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-5 mb-2">
                  <div className="col-span-3 role-header">Role</div>
                  <div className="col-span-2 text-right role-header">Hourly Rate</div>
                </div>
                {roles.map((role) => (
                  <RoleInput 
                    key={role.id} 
